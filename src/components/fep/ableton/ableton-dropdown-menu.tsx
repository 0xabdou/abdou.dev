...
const AbletonDropdownMenu = ({ open }: AbletonDropdownMenuProps) => {
  return (
    <div
      className={`flex flex-col absolute inset-x-0 h-screen bg-[#0000ff] z-40 
    ...
    >
      ...
      <div className="flex relative">
        <div className="flex w-full space-x-4 overflow-x-auto overflow-y-hidden min-h-[7rem]">
          <MenuParagraph
            label="Loop"
        ...
        <div className="absolute right-0 w-16 h-full bg-gradient-to-l from-[#0000ff]" />
      </div>
    </div>
  );
};
...
const MenuItem = ({ header, label, mini }: MenuItemProps) => {
  ...
  return header ? (
    ...
    <a className={className} href={`#${label}`}>
      {label}
    </a>
  );
};
...
const MenuParagraph = ({ label, text }: MenuParagraphProps) => {
  return (
    <a
      href={`#${label}`}
      className="flex flex-col w-56 h-20 flex-shrink-0 grow-0"
    >
